...
module xi.Test {
	class TestDataCtrl {
		private aboutOptions: IAboutOptions;
		private aboutOptions2: IAboutOptions;
		private autocompleteOptions: IAutoCompleteOptions;
		private autocompleteOptionsPersons: IAutoCompleteOptions;
		private autocompleteOptionsPersons2: IAutoCompleteOptions;
		private applicationMenuOptions;
		private chartOptions: IChartOptions;
		private colorpickerOptions: IColorPickerOptions;
		private datagridOptions: IDataGridOptions;
		private datepickerOptions: IDatePickerOptions;
		private listviewOptions: IListViewOptions;
		private lookupOptions: ILookupOptions;
		private maskCurrencyOptions: IMaskOptions;
		private maskPhoneOptions: IMaskOptions;
		private maskDateOptions: IMaskOptions;
		private maskThousandsOptions: IMaskOptions;
		private maskMustCompleteOptions: IMaskOptions;
		private maskCustDefOptions: IMaskOptions;
		private multiselectOptions: IMultiSelectOptions;
		private popoverOptions: ITooltipOptions;
		private popupmenuOptions: IPopupMenuOptions;
		private searchfieldOptions: ISearchFieldOptions;
		private searchfieldOptions2: ISearchFieldOptions;
		private searchfieldOptions3: ISearchFieldOptions;
		private sliderOptions: ISliderOptions;
		private textareaOptions: ITextAreaOptions;
		private timepickerOptions: ITimePickerOptions;
		private validateRequiredOptions: IValidateOptions;
		private validateDateOptions: IValidateOptions;
		private validateTimeOptions: IValidateOptions;
		private dropDownModel: string;

		private ddmodel: string;
		private ddmodel2: string;
		private dditems: string[];
		private dditems2: string[];
		private ddmodel3: string;
		private dditems3: string[];
		...
		constructor(public scope: ng.IScope) {
			...
			this.searchfieldOptions = {
				...
				'<small style="font-size: 12px !important; font-style: italic; margin-top: -3px; margin-bottom: 5px; display: block;">{{& email}}</small>' +
				'</a></li></script>',
				allResultsCallback: (result: any) => { self.testAllResults(result) }
			}
			...
			scope.$on('$includeContentLoaded', (event, url) => {
				if (url === "directive-templates/searchfield.html") {
					$("#searchfield1").on("selected", (event, target, object) => {
						scope.$apply(() => {
					...
					$("#form-searchfield").on("selected", (event, target, object) => {
						scope.$apply(() => {
		...
		private firstDropdownChange(): void {
		...
		private secondDropdownChange(): void {
		...
		private removeSelected() {
			const listviewApi = $("#multiselect-listview").data("listview");
			const selectedItems = listviewApi["selectedItems"];
		...
		private addToDropDown() {
			this.scope["dropDownOptions"].push("seventh");
		}

		private changeDropDownValue() {
